...
def findBlueBnt(image):
    try:
        ...
        bluecircle = cv2.HoughCircles(bluemask, cv2.HOUGH_GRADIENT, 1, minDist=5000, param1=1, param2=1, minRadius=5, maxRadius=9)
        #print("blue circle" + str(bluecircle))
...
def findRedCircels(image):
    try:
        ...
        redcircle = cv2.HoughCircles(redmask, cv2.HOUGH_GRADIENT, 1, minDist=5000, param1=1, param2=1, minRadius=5, maxRadius=9)
        #print("redcircle " +str(redcircle) )
        redconnectorbig = cv2.HoughCircles(redmask, cv2.HOUGH_GRADIENT, 1, minDist = 5000, param1 = 50, param2 = 10, minRadius= 9, maxRadius=18)
        #print("redconnector big"  +str(redconnectorbig) )
        #redconnectorsmall = cv2.HoughCircles(redmask, cv2.HOUGH_GRADIENT, 1, minDist = 5000, param1 = 50, param2 = 10, minRadius= 1, maxRadius=5)
        ...
        if redconnectorbig is not None:
...
def caculateMask(bluecircle, redcircle, angle, image):
    ...
    try:
        ...
        if bluecircle is not None:
            ...
            x1 = bluecircle[0][0] + 42 * np.cos(np.radians(angle+43))
            y1 = bluecircle[0][1] + 42 * np.sin(np.radians(angle+43))
            x2 = bluecircle[0][0] + 133* np.cos(np.radians(angle-75.5))
            y2 = bluecircle[0][1] + 133 * np.sin(np.radians(angle-75.5))
            x3 = bluecircle[0][0] + 285 * np.cos(np.radians(angle-83.5))
            y3 = bluecircle[0][1] + 285 * np.sin(np.radians(angle-83.5))
            x4 = bluecircle[0][0] + 320 * np.cos(np.radians(angle-117))
            y4 = bluecircle[0][1] + 320 * np.sin(np.radians(angle-117))
            x5 = bluecircle[0][0] + 200 * np.cos(np.radians(angle-138))
            y5 = bluecircle[0][1] + 200 * np.sin(np.radians(angle-138))
            x6 = bluecircle[0][0] + 150 * np.cos(np.radians(angle+173))
            y6 = bluecircle[0][1] + 150 * np.sin(np.radians(angle+173))
...
def findBlaCircels(Image, mask1, bluecircle, redcircle):
    ...
    labimg = cv2.cvtColor(mask1, cv2.COLOR_BGR2LAB)
    lowerblack = np.array([0, 126, 123])           # werte 05.05 mit black screen detection
    upperblack = np.array([50, 255, 255])  
    
    graymask1 = cv2.inRange(labimg, lowerblack, upperblack)
    ...
    #cv2.imshow("black mask  " , graymask1)
    #cv2.waitKey(0)
        
    blackcircels = cv2.HoughCircles(graymask1, cv2.HOUGH_GRADIENT, 1, minDist=80, param1=50, param2=5, minRadius=4, maxRadius=6 )
    ...
    #    print(blackcircels[i])
    #    cv2.circle(Image, (int(blackcircels[i][0]), int(blackcircels[i][1])), int(blackcircels[i][2]),(0, 255, 0), 2, cv2.LINE_AA)
    #    distance = np.linalg.norm(bluecircle-blackcircels[i])
    ...
    if blackcircels is not None:
        ...
        if bluecircle is not None:
            for blackcircle in blackcircels:
                ...
                #cv2.circle(Image, (int(blackcircle[0]), int(blackcircle[1])), int(blackcircle[2]),(0, 255, 0), 2, cv2.LINE_AA)
                #cv2.imshow("circles ", Image )
                #cv2.waitKey(0)
            ...
            #mindstances = np.array([41, 130, 142, 188, 281, 315])
            #maxidstances = np.array([47, 136, 148, 194, 287, 321])
            mindstances = np.array([38, 127, 139, 185, 279, 313])
            maxidstances = np.array([50, 139, 151, 201, 290, 325])
            i = 0
            ...
            for i in range(6):
                ...
                #print("pos " +str(pos))
                while(j < 6):
                    if mindstances[j] < distances[pos] and maxidstances[j] > distances[pos]:
                        sortedpoints[j][:] = blackcircels[pos][:]
                        distances[pos] = 10000
                        temp = j
                        j = j+1
                        #print("sorted points" + str(j) + str(sortedpoints))
                        break
                    elif j == 5 and pos != 5:
                        distances[pos] = 10000
                        j = temp
...
def caculatePosition2Base(pic, real, image):
    print("bildpunkte" + str(pic))
    print("objektpunkte" + str(real))
    ...
    Kmat = np.array([[1057.39,0,1099.0699-690],[0,1056.4,646.316-330],[0,0,1]])
    # [330:820, 690:1430]
    Dmat = np.array([-0.0499,0.0141,-0.0,-0.0003,-0.0061])
...
def BoardDetetction():
    ...
    zedImage = bridge.imgmsg_to_cv2(zedBuf, desired_encoding='passthrough')
    #697,334 / 1422,336  / 1423,820 / 693,817 --> 490*740
    crop_img = zedImage[330:820, 690:1430]

    #cv2.imwrite("/home/robothon/Downloads/crop.jpg", crop_img)    
    idsImage = crop_img
    loadImage = crop_img.copy()
    ...
    #blackcircels = findBlaCircelsLab(idsImage, loadImage)
    ...
    cv2.imwrite("/home/robothon/Documents/BoardDetection/detection.png",idsImage)
...
if __name__ == "__main__":
    #Setup Ros
    rospy.init_node('board_detection_zed')
    ...
    rospy.Subscriber("/zed2/zed_node/right/image_rect_color", sensor_msgs.msg.Image, zedImgCallBack)
    s = rospy.Service("/board_detection_zed", GetBoardLocation, service_callback)
    ...
    while(not rospy.is_shutdown()):
        ...
        elif zed == True and detection == True:
