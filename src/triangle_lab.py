...
def service_callback(req):
    ...
    if(screenMask is not None):
        ...
        manual_mask[y+manual_region[1]:y+manual_region[3], x+manual_region[0]:x+manual_region[2]] = 0  # Region within the bounding rectangle
         # Apply the manual mask to the screenMask
        cv2.imshow("manual_mask", manual_mask) # for debugging
        screenMask = cv2.bitwise_and(screenMask, screenMask, mask=manual_mask)
        # rospy.logwarn("Screen detected", screenMask.size(   ))
        ...
        if(positions[0] is None): 
            ...
            if(positions[2] is None): #no green Triangle: Delta between red and yellow
                #rospy.logwarn("no green Triangle: Delta between red and yellow")
                ...
                rospy.logerr_throttle(30, "Triangle Delta is: " + str(positions[1][0] - positions[2][0]))
            delta = delta/(width/2) #normalize delta to screen size
            status = 1
    else:
        print("No screen detected")
    #rospy.logwarn("Status %i", status)
...
def detectM5():
    ...
    if(realBuf[0] is not None):
        ...
        loweryellow = np.array([0, 151, 156])
        upperyellow = np.array([113, 167, 203])
...
def adjust_saturation_and_brightness(frame, saturation_scale=1.5, brightness_offset=-50):
    ...
    # Convert the HSV values to float32 for more precise manipulation
    hsv = np.float32(hsv)
    ...
    # Convert the HSV values back to uint8
    hsv = np.uint8(hsv)

    # Convert the frame back from HSV to BGR color space
