...
def optimize_transformation_matrix(C):
    C_rot_matrix = C[:3, :3]
    C_trans_vec = C[:3, 3]
...
def service_callback(req):
    global tfBuffer
    print(">> Service called") # for debugging
    if(idsBuf is not None):
        ...
        tb  = cam @ M
        tb = optimize_transformation_matrix(tb)
        print("tb: ", tb) # for debugging
...
def findRedCircels(image):
    try:
        ...
        #cv2.imshow("redmask" , cv2.resize(redmask, (1384, 923))) # for debugging
        #cv2.waitKey(500)
        ...
        if redconnectorbig is not None and redconnectorsmall is not None:
            ...
            redconnector = None
            print("NOTE: ensure the red plug is inserted in the black connector and not the red one! This might be preventing the detection.")
...
def findRedM5(image):
    try:
        ...
        #cv2.imshow("red m5" , cv2.resize(redmask, (692,462))) # for debugging
        #cv2.waitKey(0)
...
def BoardDetetction():
    ...
    print("picPoints: ", picPoints) # for debugging
...
def capture_images(n, interval):
    ...
    for i in range(n):
        idsImage = bridge.imgmsg_to_cv2(idsBuf, desired_encoding='passthrough')
        # Save the image
        image_path = os.path.join('images', f'iphone_{i}.png')
        cv2.imwrite(image_path, idsImage)
        print(f"Saved {image_path}")
        cv2.imshow("image", cv2.resize(idsImage, (1362, 923)))
...
if __name__ == "__main__":
    ...
    zed = False
    detection = False
    print(">> in main function") # for debugging
    if(detection == False):
